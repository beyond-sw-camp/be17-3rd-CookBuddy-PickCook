...
public enum BaseResponseStatus {
    ...
    EMAIL_FOUND_SUCCESS(true, 20104, "이메일을 찾았습니다."),
    ...
    INTERNAL_TOKEN_GENERATED(true, 20302, "비밀번호 변경 토큰이 생성되었습니다."),
    TOKEN_VALID(true, 20303, "유효한 토큰입니다."),
    ...
    CURRENT_PASSWORD_REQUIRED(false, 30305, "현재 비밀번호를 입력해주세요."),
    INVALID_CURRENT_PASSWORD(false, 30306, "현재 비밀번호가 일치하지 않습니다."),
    TOKEN_ALREADY_USED(false, 30307, "이미 사용된 토큰입니다."),
    ...
    RESOURCE_NOT_FOUND(false, 40003, "요청한 리소스를 찾을 수 없습니다."),
    ...
    // 상품 기본 오류
    PRODUCT_NOT_FOUND(false, 40110, "존재하지 않는 상품입니다."),
    PRODUCT_TITLE_EMPTY(false, 40111, "상품명을 입력해주세요."),
    PRODUCT_PRICE_INVALID(false, 40112, "상품 가격은 0보다 커야 합니다."),
    PRODUCT_DISCOUNT_RATE_INVALID(false, 40113, "할인율은 0~100 사이의 값이어야 합니다."),
    PRODUCT_DELETED(false, 40114, "삭제된 상품입니다."),

    // 상품 필터링 관련 오류
    CATEGORY_ID_INVALID(false, 40120, "유효하지 않은 카테고리 ID입니다."),
    CATEGORY_PRODUCTS_NOT_FOUND(false, 40121, "해당 카테고리에 상품이 없습니다."),
    CATEGORY_ACCESS_DENIED(false, 40122, "카테고리에 접근할 권한이 없습니다."),
    INVALID_PAGE_PARAMETER(false, 40123, "유효하지 않은 페이지 번호입니다."),
    INVALID_SIZE_PARAMETER(false, 40124, "페이지 크기는 1~100 사이의 값이어야 합니다."),
    INVALID_SORT_PARAMETER(false, 40125, "유효하지 않은 정렬 조건입니다."),
    FILTER_QUERY_TOO_COMPLEX(false, 40126, "필터 조건이 너무 복잡합니다."),

    // 상품 필터링 관련 오류 (기존 코드 아래에 추가)
    INVALID_SORT_FIELD(false, 40127, "지원하지 않는 정렬 필드입니다."),
    INVALID_SORT_DIRECTION(false, 40128, "정렬 방향은 ASC 또는 DESC만 가능합니다."),
